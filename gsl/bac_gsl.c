...
int bac_rhs(double t, const double y[], double f[], void *params){
	...
	for(j=0; j<TYPES;++j){
		/*births*/
		birth=(1.-mu)*(1.-cost*s[j]*inv[j])*beta*y[j];//division of type j
		jpl=j+1;
		jmi=j-1;
		if(j==0){
			jmi=jpl;
		...
		birth+=0.5*mu*(1.-cost*s[jpl]*inv[jpl])*beta*y[jpl];//division of type j+1 -> mutation into j
		birth+=0.5*mu*(1.-cost*s[jmi]*inv[jmi])*beta*y[jmi];//division of type j-1 -> mutation into j
		/*death*/
		death=delta*micr[idh]*y[j];
...
int bac_jac(double t, const double y[], double *dfdy,double *dfdt, void *params){
	...
	for(b=0; b<TYPES; ++b){
		...
		dfdy[idj1]+=0.5*mu*beta*(1.-cost*s[bpl]*inv[bpl]);//supradiagonal jac term
		
		dfdy[idj2]+=0.5*mu*beta*(1.-cost*s[bmi]*inv[bmi]);//subdiagonal jac term
...
int bacDynamics(gsl_odeiv2_driver *driver,SysParams *spar,Event *event){
	int i,j,k,idh,idk,nv,nh,status;
	...
	//updating the time variable which (struct @event stores the current time on @timeE)
	event->timeE=time_tmp;
